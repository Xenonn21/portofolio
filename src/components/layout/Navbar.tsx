...
// Scroll ke section tanpa mengubah URL hash (hindari focus trap dari href="#")
...
// Hitung section mana yang paling banyak pixel-nya terlihat di viewport
function getMostVisibleSection(): SectionId {
  ...
  for (const { id } of navItems) {
    ...
    const px = Math.max(0, Math.min(rect.bottom, vh) - Math.max(rect.top, 0));
    if (px > bestPx) { bestPx = px; bestId = id as SectionId; }
  }
  return bestId;
}
...
export default function Navbar() {
  ...
  // ── Active section via scroll (most-visible pixel) ─────────────────────
  useEffect(() => {
    const onScroll = () => {
      const next = getMostVisibleSection();
  ...
  // ── Click handler — set immediately, then smooth scroll ───────────────
  const handleNav = (id: SectionId) => {
    setActiveSection(id);   // update pill seketika
    scrollToSection(id);    // smooth scroll tanpa href hash
  };

  // ── Theme ──────────────────────────────────────────────────────────────
  const navBg          = isLight ? "border-black/10 bg-white/80"      : "border-white/10 bg-black/50";
  ...
  const pillBg         = isLight ? "bg-purple-500/10 border-purple-500/20" : "bg-purple-500/20 border-purple-500/30";
      ...
      <header className="fixed top-5 left-0 z-50 w-full px-6">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <button
            ...
            className={`flex items-center gap-3 rounded-full border p-1 backdrop-blur transition-colors duration-300 ${navBg}`}
          >
